...
public class CpRolesResource {

	private final Logger log = LoggerFactory.getLogger(CpRolesResource.class);
	...
	@ResponseBody
	public ResponseEntity<List<RoleDTO>> getAll(HttpServletRequest request) throws InvalidAPIKeyException {
		...
		try {
			result = restTemplate.exchange(cpApplicationProperties.getCpRoleUri(), HttpMethod.GET, getEmployeeRequest,
					responseType);
			...
			result = new ResponseEntity<List<RoleDTO>>(HttpStatus.SERVICE_UNAVAILABLE);
			throw new HttpServerErrorException(result.getStatusCode());
		}
	...
	/**
	 * DELETE /role/:id -> delete the "id" role.
	 */
	@RequestMapping(value = "/roles", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<Object> deleteRoleBasedOnId(HttpServletRequest request, @RequestParam("id") String roleId)
			throws InvalidAPIKeyException {
		log.debug(String.format("Rest request to get roles based on roleId:%s ", roleId));
		MultiValueMap<String, String> params = new LinkedMultiValueMap<String, String>();
		...
		try {
			result = restTemplate.exchange(cpApplicationProperties.getCpRoleUri() + urlParam, HttpMethod.DELETE,
