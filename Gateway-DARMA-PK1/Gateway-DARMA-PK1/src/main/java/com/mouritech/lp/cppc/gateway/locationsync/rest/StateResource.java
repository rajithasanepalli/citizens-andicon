...
import com.mouritech.lp.cppc.gateway.locationsync.dto.StateDTO;
...
public class StateResource {
	...
	/**
	 * GET /state -> get all the state.
	 */
	@RequestMapping(value = "/state", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<List<StateDTO>> getAll(HttpServletRequest request) throws InvalidAPIKeyException {
		...
		ParameterizedTypeReference<List<StateDTO>> responseType = new ParameterizedTypeReference<List<StateDTO>>() {
		};
		ResponseEntity<List<StateDTO>> result = null;
		try {
			result = restTemplate.exchange(applicationProperties.getStateUri(), HttpMethod.GET, getEmployeeRequest,
					responseType);
