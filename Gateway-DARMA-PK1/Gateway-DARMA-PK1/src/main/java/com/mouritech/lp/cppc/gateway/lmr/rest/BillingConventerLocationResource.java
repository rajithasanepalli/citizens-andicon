...
public class BillingConventerLocationResource {

	private final Logger log = LoggerFactory.getLogger(BillingConventerLocationResource.class);
	@Autowired
	 ...
	 */
	@RequestMapping(value = "bc/location", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<List<LocationDTO>> getLocationForBillingConv(
			HttpServletRequest request)
			throws InvalidAPIKeyException {
		log.debug(String.format(
				"Rest request to get employees based on locationId:%s "));
		...
		try {
			result = restTemplate.exchange(
					applicationProperties.getBillingConvUri(),
