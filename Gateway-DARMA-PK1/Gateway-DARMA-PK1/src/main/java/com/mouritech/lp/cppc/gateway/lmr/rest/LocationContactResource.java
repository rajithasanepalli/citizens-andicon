...
import com.mouritech.lp.cppc.gateway.DTO.LocationContactDTO;
...
public class LocationContactResource {
	...
	private final Logger log = LoggerFactory
			.getLogger(LocationContactResource.class);
	@Autowired
	private LmrApplicationProperties applicationProperties;

	/**
	 * POST /LocationMoreInfo -> Create a new locationContact.
	 */
	@RequestMapping(value = "/locationContact", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Object> create(
			@Validated @RequestBody LocationContactDTO locationContactDTO,
			HttpServletRequest request) throws InvalidAPIKeyException,
			URISyntaxException {
		log.debug("[locationContact] REST request to recieved from  :",
				request.getRemoteAddr());
		log.debug(
				"[locationContact] REST request to save locationContact : {}",
				locationContactDTO);
		...
		// locationContactDTO.setSessionId(request.getHeader("x-session-id"));
		// locationContactDTO.setApproverId(applicationProperties.getApi_key());
		headers.add("Content-Type", request.getHeader("content-type"));
		...
		HttpEntity<LocationContactDTO> reuestbody = new HttpEntity<LocationContactDTO>(
				locationContactDTO, headers);

		ResponseEntity<Object> result = null;

		try {
			result = restTemplate.exchange(
					applicationProperties.getLocationContactsUri(),
	...
	/**
	 * PUT /locationContact -> update a new locationContact.
	 */
	...
	public ResponseEntity<Object> updateLocationMoreInfo(
		...
		ResponseEntity<LocationContactDTO> result = null;

		try {
			result = restTemplate.exchange(
					applicationProperties.getLocationContactsUri(),
					HttpMethod.PUT, reuestbody, LocationContactDTO.class);
			...
			result = new ResponseEntity<LocationContactDTO>(
	...
	@ResponseBody
	public ResponseEntity<LocationContactDTO> getLocationContactOnId(
			HttpServletRequest request,
			@RequestParam("id") String locationContactId)
			throws InvalidAPIKeyException {
		log.debug(String.format(
				"Rest request to get locationContact based on locationContactId:%s ",
				locationContactId));
		...
		try {
			result = restTemplate.exchange(
					applicationProperties.getLocationContactsUri() + urlParam,
					HttpMethod.GET, getEmployeeRequest,
					LocationContactDTO.class);
	...
	@ResponseBody
	public ResponseEntity<List<LocationContactDTO>> getAll(
			HttpServletRequest request,
			...
			throws InvalidAPIKeyException {
		log.debug(String
				.format("Rest request to get locationContact based on page:%s and Per_page:%s ",
						page, perPage));
		MultiValueMap<String, String> params = new LinkedMultiValueMap<String, String>();
		...
		ParameterizedTypeReference<List<LocationContactDTO>> responseType = new ParameterizedTypeReference<List<LocationContactDTO>>() {
		};

		ResponseEntity<List<LocationContactDTO>> result = null;
		try {
			result = restTemplate.exchange(
					applicationProperties.getLocationContactsUri() + urlParam,
			...
			result = new ResponseEntity<List<LocationContactDTO>>(
					HttpStatus.SERVICE_UNAVAILABLE);
			throw new HttpServerErrorException(result.getStatusCode());
		}
	...
	@ResponseBody
	public ResponseEntity<Object> deleteLocationContactonId(
			HttpServletRequest request,
			@RequestParam("id") String locationContactId)
			throws InvalidAPIKeyException {
		log.debug(String.format(
				"Rest request to delete locationContact based on locationContactId:%s ",
				locationContactId));
		MultiValueMap<String, String> params = new LinkedMultiValueMap<String, String>();
